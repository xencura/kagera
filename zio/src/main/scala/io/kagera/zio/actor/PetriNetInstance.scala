...
class PetriNetInstance[S](
  ...
  override def receive[A](state: State, msg: Message[A], context: Context): RIO[Any, (Command[Event], State => A)] =
    state match {
      case None => uninitializedPetriNet(msg)
      case Some(instance) => initializedPetriNet(instance, msg, context)
    }
  def uninitializedPetriNet[A](msg: Message[A]): UIO[(Command[Event], State => A)] = msg match {
    case in: Initialize[S] =>
      UIO(
        (
          Command.persist(InitializedEvent(in.marking, in.state)),
          (s: State) => s.map(i => Initialized(i.marking, i.state)).get
        )
      )
    case _: Message[Response] =>
      UIO((Command.ignore, _ => IllegalCommand("Only accepting Initialize commands in 'uninitialized' state")))
  }
  def initializedPetriNet[A](
    instance: Instance[S],
    msg: Message[A],
    context: Context
  ): Task[(Command[Event], State => A)] = msg match {
    ...
    case _: Initialize[_] =>
      UIO((Command.ignore, _ => IllegalCommand("Already initialized")))
