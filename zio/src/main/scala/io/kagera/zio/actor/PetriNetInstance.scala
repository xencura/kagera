...
object PetriNetInstance {
  ...
  def props[S](topology: ExecutablePetriNet[S], persistenceId: PersistenceId, settings: Settings = defaultSettings, config: Config) =
...
class PetriNetInstance[S](
  ...
  config: Config
) extends EventSourcedStateful[Clock, Option[Instance[S]], Message, Event](persistenceId, EventSourcedStateful.retrieveJournal(JournalPluginClass("zio.actors.persistence.journal.InMemJournal"), _)) {
