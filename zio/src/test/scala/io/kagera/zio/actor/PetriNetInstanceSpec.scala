    ...
    suite("A persistent petri net actor")(
      testM("Respond with an Initialized response after processing an Initialized command") {
        ...
        for {
          ...
          res <- actor ? Initialize(initialMarking, Set(1, 2, 3))
        } yield assert(res)(equalTo(Initialized(initialMarking, Set(1, 2, 3))))
      } @@ timeout(60.seconds),
      testM("Before being intialized respond with an IllegalCommand message on receiving a GetState command") {
        val sequenceNet = new TestSequenceNet {
          override val sequence = Seq(transition()(_ => Added(1)), transition()(_ => Added(2)))
        }
        import sequenceNet._
        ...
        } yield assert(res)(isSubtype[IllegalCommand](anything))
      } @@ timeout(60.seconds),
      testM("Afer being initialized respond with an InstanceState message on receiving a GetState command") {
        val sequenceNet = new TestSequenceNet {
          override val sequence = Seq(transition()(_ => Added(1)), transition()(_ => Added(2)))
        }
        import sequenceNet._
        for {
          ...
          res1 <- actor ? Initialize(initialMarking, Set(1, 2, 3))
      ...
      testM("Respond with a TransitionFailed message if a transition failed to fire") {
        ...
        for {
          ...
          res1 <- actor ? Initialize[Set[Int]](initialMarking, Set.empty)
          res2 <- actor ? FireTransition(1, ())
        } yield assert(res1)(isSubtype[Initialized[_]](anything)) && assert(res2)(isSubtype[TransitionFailed](anything))
      } @@ timeout(60.seconds),
      testM(
        ...
        for {
          ...
          _ <- actor ! Initialize(initialMarking, Set.empty)
          res1 <- actor ? FireTransition(1, ())
          res2 <- actor ? FireTransition(1, ())
        } yield assert(res1)(isSubtype[TransitionFailed](hasTransitionId(1L))) && assert(res2)(
          isSubtype[TransitionNotEnabled](hasTransitionId(1L))
        )
      } @@ timeout(60.seconds),
      testM(
        ...
        } yield assert(res)(isSubtype[TransitionNotEnabled](hasTransitionId(transitionId)))
      } @@ timeout(60.seconds),
      /*
      ...
      testM("Be able to restore it's state from persistent storage after termination") {
        ...
        for {
          ...
          init <- actor ? Initialize(initialMarking, Set.empty)
          ...
          assert(res2)(equalTo(InstanceState[Set[Int]](3, Marking(place(3) -> 1), Set(1, 2), Map.empty))) &&
          // assert that the actor is the same as before termination
          assert(res3)(equalTo(InstanceState[Set[Int]](3, Marking(place(3) -> 1), Set(1, 2), Map.empty)))
        }
